...
> 最后更新：2026-07-11（补充 createSceneScaleFromDesignCenter + 修复 computeScale 引用）
...
> **豁免**：通过 `createSceneScaleFromDesignCenter` 构造的 `sceneScale` 内部使用相同公式，
> 但工厂函数封装了对齐保证（sceneScale 与 ballPos 使用同一组参数），这是允许的。
