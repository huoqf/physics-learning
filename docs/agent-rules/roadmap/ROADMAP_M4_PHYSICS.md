...
- [ ] **CuttingEMF 增强**（[M4-1.x] 路线图追加 2026-06-02）

  > 缘由：[M4-1] 收官后，为深化高考必考点 `electricity-4-3` 教学效果，对 CuttingEMF 进行单点增强。
  > 范围：仅扩展 CuttingEMF 单组件，不影响其他 39 个动画 chunk。
  > 不引入新 npm 依赖；新 SVG 资产走 inline 路线并在 `process/PROCESS_LOG.md` 申报。

  - [x] **参数扩展**：`theta` (0~90°，默认 90° 对应 sinθ=1)、`r` (内阻 0~2Ω，默认 0)、`B_out` (方向切换 0=向里⊗ 1=向外⊙)
  - [x] **物理真实化**：取消 ping-pong 镜像，v∈[-5, 5] m/s 支持真实反向切割；棒到达导轨端点自动停止
  - [x] **磁场符号方向化**：`B_out=0` 显示 ⊗ 向里、`B_out=1` 显示 ⊙ 向外；EMF 箭头方向按右手定则自动翻转
  - [x] **公式更新**：`EMF=BLv·sinθ`、`I=EMF/(R+r)`、`F安=B²L²v·sinθ/(R+r)` 同步计算与公式面板
  - [x] **探索任务卡**（[PR 4]）：3 个递进引导
    - 探索一（修订）：B↑、v↑ → 灯泡变亮（**避免"减 R 最亮"误区**）
    - 探索二：v 反向 → I/F_安 同步反向（楞次定律直观）
    - 探索三：切换 B_out → 电子受力立即反向
  - [x] **PiP 放大镜**（[PR 3] ✅ 2026-06-02）：自由电子 + 洛伦兹力 F洛=qvB
  - [x] **真实右手定则 SVG**（[PR 3] ✅ 2026-06-02）：inline 路径手绘，无新依赖
  - [x] **交互模式三态**（[PR 2] ✅ 2026-06-02）：手动拖拽 / 匀速 / 受力 状态机

  **风险与豁免**：Canvas 7 元素限制豁免 ⊗ 网格（视为"参考层"）+ PiP 独立子画布；探索一目标修改需在 `data/knowledgeTree.ts` 的 `electricity-4-3` 节点的 problemIds 预留接入位。
