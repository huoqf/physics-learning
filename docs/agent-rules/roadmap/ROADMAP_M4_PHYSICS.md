...
- [ ] **CuttingEMF 增强**（[M4-1.x] 路线图追加 2026-06-02）
  ...
  - [ ] **参数扩展**：`theta` (0~90°，默认 90° 对应 sinθ=1)、`r` (内阻 0~2Ω，默认 0)、`B_out` (方向切换 0=向里⊗ 1=向外⊙)
  - [ ] **物理真实化**：取消 ping-pong 镜像，v∈[-5, 5] m/s 支持真实反向切割；棒到达导轨端点自动停止
  - [ ] **磁场符号方向化**：`B_out=0` 显示 ⊗ 向里、`B_out=1` 显示 ⊙ 向外；EMF 箭头方向按右手定则自动翻转
  - [ ] **公式更新**：`EMF=BLv·sinθ`、`I=EMF/(R+r)`、`F安=B²L²v·sinθ/(R+r)` 同步计算与公式面板
  - [ ] **探索任务卡**（[PR 4]）：3 个递进引导
