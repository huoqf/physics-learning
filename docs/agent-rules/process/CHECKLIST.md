...
- [ ] **新页面布局**：使用 `useAnimationViewport({ preset })` + `AnimationSvgCanvas`（详见 `project_rules.md §4.8`）
- [ ] **坐标转换**：新页面使用 `worldToDesign()`（`src/scene`）+ `useSceneScale`；Canvas 路径使用 `useCanvasViewport` + `designToPixel`；禁止手写坐标计算
- [ ] **交互拖拽坐标转换**：使用 `useViewportPointer(svgRef)` 原生矩阵逆变换映射；严禁在业务层散落 `clientX - rect.left - vp.tx` 等硬算公式
