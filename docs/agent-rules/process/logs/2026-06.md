...
### 万有引力与人造卫星硬编码治理及轨道配色规范化（归属 [M2] 架构完善与全局规范 · 知识点 mechanics-6-2/3）

**核心改动：**

1. **明确硬编码治理作用域 (不伤及 Theme Token)**
   - 物理常数与颜色的治理作用域严格局限在 `GravityAnimation.tsx` 与 `SatelliteAnimation.tsx` 视图组件中的局部字面量。保证不伤及在 `src/theme/`（如 `colors.ts`、`sceneColors.ts`）中正常声明的系统级样式 Theme Token。

2. **新增地球海洋渐变 Token**
   - 在 `src/theme/physics/sceneColors.ts` 的 `COMMON_MATERIALS` 材质集合中，专门增设了专用于天体力学地球渲染的 `earthOceanGrad` 径向色阶 `['#DBEAFE', '#93C5FD', '#1E3A8A'] as const`，从而消除了原本的 `#1e3a8a` 海洋色硬编码，避免了从钢质球体色阶借用色彩所造成的语义污染。

3. **物理常数全局统一**
   - 在 `src/physics/constants.ts` 中导出地球半径物理常量 `EARTH_RADIUS = 6.37e6`。
   - 在 `SatelliteAnimation.tsx` 视图中删除自行声明的 `G`、`M_earth`、`R_earth` 常数，统一从 `@/physics/constants` 导入 `GRAVITATIONAL_CONSTANT`、`EARTH_MASS`、`EARTH_RADIUS`，消除了常数多头硬编码，保持物理计算与看板的精度一致。

4. **图纸卡片阴影硬编码消除**
   - 在 `GravityAnimation.tsx` 和 `SatelliteAnimation.tsx` 的 `<defs>` 中，用 SVG 标准阴影滤镜 `<filter id="card-shadow">` 替换了原本内联在 `<rect>` 上硬编码的 `rgba(0,0,0,0.12)` 滤镜，实现了无字面量阴影特效。

5. **轨道用色及绘制参数（线宽、虚实、透明度）统一规范化**
   - 将对比背景轨道、当前主轨、发射参考圈和发射预测线的 `stroke` 颜色统一绑定为 `PHYSICS_COLORS.trackHistory`，消除了之前混合借用 `PHYSICS_COLORS.axis` 的不统一现象。
   - 建立模块级静态 `LAYOUT` 参数对象，收拢所有轨道线的样式参数（如 `strokeWidth`、`strokeDasharray`、`opacity`）到 `LAYOUT.orbit` 样式包中，确保 SVG 渲染内联样式中完全不残留任何硬编码数字。

6. **补全 KeplerAnimation.tsx 主题导出**
   - 修复了 `src/theme/physics/index.ts` 桶文件中漏掉二次导出 `KEPLER_CONFIG`、`VECTOR_DISPLAY`、`INSET_CHART` 和 `GRID_DISPLAY` 导致 KeplerAnimation.tsx 编译失败的隐患，全项目 `tsc --noEmit` 0 错误通过。

7. **单元测试与验证**
   - 跑通 `celestial.test.ts` 中 10 项天体力学公式及发射轨道回归测试，100% 测试通过。

**涉及文件：**
- `src/physics/constants.ts`
- `src/theme/physics/sceneColors.ts`
- `src/theme/physics/index.ts`
- `src/features/mechanics/GravityAnimation.tsx`
- `src/features/mechanics/SatelliteAnimation.tsx`

### 人造卫星物理教学改进及优化、美化开发（归属 [M1] 力学动画 与 [M2] 架构完善 · 知识点 mechanics-6-3）

**核心改动：**

1. **物理计算层新增纯函数**（`src/physics/celestial.ts`）
   - 在 `celestial.ts` 中新增 `calculateLaunchTrajectory` 纯函数，通过极坐标圆锥曲线二体方程求解卫星发射的轨道参数，包括偏心率 $e$、半正焦弦 $p$、近地点半径 $r_{\text{peri}}$ 及远地点半径 $r_{\text{apo}}$，自动区分并返回轨道类型（坠毁、近地圆、椭圆、逃逸），将复杂的物理解析独立于渲染流程。

2. **物理看板数据层重构与下沉**（`src/data/physicsQuantities.ts`）
   - 重构 `case 'anim-satellite'` 分支。根据多轨道圆周对比模式 (mode 0) 和宇宙速度发射模式 (mode 1)，通过 `calculateOrbitalSpeed` 和 `calculateLaunchTrajectory` 分别解出高精度的线速度、运行周期、向心加速度、轨道类型、偏心率及近/远地点半径等物理量。
   - 彻底清除 Canvas 中所有的硬编码文本，下沉为右侧看板的高质量 LaTeX 公式及高考核心考点卡片。

3. **参数配置升级与注册**（`src/data/animationRegistry.ts`）
   - 扩展 `anim-satellite` 注册元数据，包括 `mode`, `v0`, `isLaunched`, `showChart`, `showCompare` 控制参数，限制滑块数量，支持条件渲染（仅发射模式可见发射和速度选择），并延迟加载 `SatelliteSidebar` 交互组件。

4. **新建侧边栏交互组件**（`src/features/mechanics/SatelliteSidebar.tsx`）
   - 提供“多轨道对比 (圆轨道)”与“发射宇宙速度 (圆锥曲线)”双模式切换 Tab 页签。
   - 发射模式下提供“发射 / 重置”控制按钮和 7.9km/s, 10.0km/s, 11.2km/s 快捷发射速度选择组件。

5. **重构主画布渲染与交互体系**（`src/features/mechanics/SatelliteAnimation.tsx`）
   - **极坐标时间积分推进**：在发射模式下，根据角动量守恒在时间轴上用 $d\theta/dt = h/r^2$ 极坐标方程步进，完美模拟近日点快、远日点慢的二体物理规律；速度低于环绕速度时滑出抛物线并撞击地球坠毁爆炸，等于或大于第二宇宙速度时沿双曲线逃逸。
   - **多轨同屏公转与手势拖动**：多轨道对比模式下渲染三条参考轨道及同屏公转的三颗不同高度卫星（验证开普勒第三定律）。支持在 Canvas 画布上直接手势拖动小卫星来调距。
   - **精密画中画与自适应图纸**：右上角渲染毛玻璃自适应图表卡片，绘制 $v(r)$（经典蓝）与 $T(r)$（紫色）曲线。图表热区支持直接在图表内**左右拖拽鼠标**反向驱动修改轨道半径。与 Canvas 卫星手势拖动互锁，完全对齐物理圆周运动图像规范。
   - **材质与色彩 100% 零硬编码**：地球采用发光大气圈和大陆科技剪影的 3D 拟物渐变，卫星绘制太阳翼电池片，引力箭头采用规范的引力绿（`PHYSICS_COLORS.gravity`），长度和粗细符合物理表现。

6. **补全天体物理计算单元测试**（`tests/physics/celestial.test.ts`）
   - 在测试文件中完整覆盖天体物理涉及的 6 大核心计算函数，包括三项宇宙速度、向心加速度、开普勒第三定律反推、行星密度及发射轨道判定等。Vitest 100% 通过（10 项测试全部跑通）。

7. **TypeScript 严格类型检查**
   - 运行 `npx tsc --noEmit` 进行 TypeScript 严格类型检查，无任何编译错误或警告。

**涉及文件：**
- `src/physics/celestial.ts`
- `src/data/physicsQuantities.ts`
- `src/data/animationRegistry.ts`
- `src/features/mechanics/SatelliteSidebar.tsx`
- `src/features/mechanics/SatelliteAnimation.tsx`
- `tests/physics/celestial.test.ts`

### 万有引力定律物理教学改进及优化、美化开发（归属 [M1] 力学动画 与 [M2] 架构完善 · 知识点 mechanics-6-2）

**核心改动：**

1. **物理看板数据层扩展与下沉**（`src/data/physicsQuantities.ts`）
   - 重构 `case 'anim-gravity'` 数据生成分支，使其支持相对单位比例验证和真实尺度探索双模式。
   - 真实尺度模式下，基于地月、日地、同步卫星、空间站宇航员 4 个常见系统预设，以规范的科学计数法形式输出各天体的质量、距离与真实的万有引力数值。
   - 精简高考要点，字数限制在 30 字以内以对齐三屏联动规范。

2. **更新动画注册表配置**（`src/data/animationRegistry.ts`）
   - 注册侧边栏 `GravitySidebar` 延迟加载项。
   - 在 `anim-gravity` 默认参数中补全 `mode`, `preset`, `showChart` 参数，并限制相对间距 `r` 在 `[1.5, 18.0]` 之间防越界，支持滑块的条件渲染（`showIf` 过滤）。

3. **新建侧边栏交互组件**（`src/features/mechanics/GravitySidebar.tsx`）
   - 提供“比例验证 (相对数值)”与“天体探索 (真实尺度)”的双模式切换 Tab 按钮。
   - 真实尺度下提供天体系统一键切换，并附带科学友好的气泡提示，完全符合左侧只提供参数和预设控制、数据向看板解出的要求。

4. **重构主画布实现多维交互与精密画中画**（`src/features/mechanics/GravityAnimation.tsx`）
   - **天体手势拖拽交互**：在 SVG 上增加鼠标移动事件处理器，支持用户通过在画面中直接拖拽天体 1 或天体 2 来平滑改变相对间距 `r`，并将 `r` 自动约束在 `[1.5, 18.0]` 范围内。在真实尺度预设下拖动时，系统能够自动将预设重置为自定义模式，实现双向受控联动。
   - **画中画平方反比 F-r 曲线图（对齐圆周运动进阶模式）**：
      - **视觉升级**：将原深色背景板重置为毛玻璃淡色悬浮卡片（采用 `SCENE_COLORS.labels.glassPanelBg`，rx=8圆角及 drop-shadow 浮动阴影）。所有文字、轴线和刻度颜色完全绑定为 `CHART_COLORS`（如 `titleText`、`labelText`、`axisLine` 等），100% 零硬编码。
      - **精密刻度与标注**：绘制了 X 轴与 Y 轴细部刻度线，X 轴两端清晰标注起止数值 `1.5` 与 `18.0`，Y 轴标出归一化极大/极小引力刻度与关系公式。
      - **双向反向拖动交互**：图表区域集成鼠标拖拽热区，支持直接在图表内**左右拖动鼠标**反向逆解出轨道距离并实时驱动全局轨道距离 `r` 变化。与 Canvas 天体手势拖拽互锁，真实尺度下拖动自动切回自定义模式，实现手势、滑块、Canvas与游标的四向无缝联动。
   - **引力场向心流动涟漪动效**：彻底抛弃了静态同心圆，改用基于 store 中 `time`（播放状态）实时解算的向内收缩淡出波纹，平滑模拟引力场的场源效应，富有动态拟物感。
   - **100% 零硬编码色彩与色彩合规治理**：清除原有天体的 HEX 十六进制硬编码，分别复用 `SCENE_COLORS.materials.steelSphereGrad`（灰色金属）与 `SCENE_COLORS.materials.vacuumSphereGrad`（蓝色钢体）进行立体渐变渲染；力矢量完全采用重力深绿色（`PHYSICS_COLORS.gravity`），长度逻辑和粗细完全符合规范。

5. **编写万有引力计算单元测试**（`tests/physics/dynamics.test.ts`）
   - 导入并添加 `calculateGravitation` 万有引力定律物理计算函数的单元测试，验证在地月尺度下引力计算的正确性，Vitest 测试 100% 通过。
...
- `src/data/physicsQuantities.ts`
- `src/data/animationRegistry.ts`
- `src/features/mechanics/GravitySidebar.tsx`
- `src/features/mechanics/GravityAnimation.tsx`
- `tests/physics/dynamics.test.ts`

### 开普勒定律物理教学改进及优化、美化开发（归属 [M1] 力学动画 与 [M2] 架构完善 · 知识点 mechanics-6-1）

**核心改动：**

1. **物理计算核心修正与开普勒轨道模拟下沉**（`src/physics/celestial.ts`）
   - 新增 `solveKeplerEquation` 偏近点角数值求解函数，使用牛顿迭代法快速精确逼近 $E - e \sin E = M$。
   - 新增 `calculateKeplerOrbit` 纯函数，根据时间与周期，计算出符合万有引力定律与面积定律的真实椭圆轨道位置坐标 $(x, y)$ 与切线速度分量 $(vx, vy)$。

2. **更新动画注册表配置**（`src/data/animationRegistry.ts`）
   - 升级 `anim-kepler` 的默认参数与参数元数据，加入 `mode` 定律模式字段以及双轨道对比所需的 `a2`（外长半轴）、`b2`（外短半轴）控制参数。
   - 注册侧边栏 `KeplerSidebar` 延迟加载项。

3. **瘦身与对齐侧边栏交互组件**（`src/features/mechanics/KeplerSidebar.tsx`）
   - 彻底移除了侧边栏中的物理量看板、公式和要点展示，将模式切换重构为紧凑的三等分网格按钮组（与 `EquilibriumSidebar` 的 2x2 grid 对齐）。

4. **物理看板数据层重构与下沉**（`src/data/physicsQuantities.ts`）
   - 新增 `case 'anim-kepler'` 分支。将物理量（长/短半轴、近日/远日距、瞬时速度等 $\le 5$ 行）、物理公式以及高考要点（$\le 3$ 条）统一收拢并下沉到 `physicsQuantities`，使右侧屏通过 `PhysicsPanel` 精确挂载展示，完全对齐三屏联动合规设计。

5. **重构主画布渲染与三大定律分步教学呈现**（`src/features/mechanics/KeplerAnimation.tsx`）
   - **100% 零硬编码颜色与色彩合规治理**：彻底清除了所有硬编码的 Hex、RGB 和 RGBA 颜色字符。重构太阳、行星的 3D 球体径向渐变，复用 `SCENE_COLORS.bulb` 和 `SCENE_COLORS.magnet` 中的标准色阶。所有硬编码 `rgba` 不透明色彩改由 `strokeOpacity/fillOpacity` 配合物理 Token 进行渲染。速度矢量和引力矢量精准绑定为经典蓝（`PHYSICS_COLORS.velocity`）和重力深绿（`PHYSICS_COLORS.gravity`）规范色。
   - **浅底对比度升级**：主画布背景调整为柔和的 `bg-neutral-50`（带边框与内阴影），对网格线、偏近点角连线、焦点及行星标签文本的颜色进行深色高对比度优化，彻底解决“看不清”的 Bug。
   - **第一定律模式**：绘制主副双焦点，随动渲染行星到双焦点的连线 $r_1, r_2$，动态验证 $r_1 + r_2 = 2a$。
   - **第二定律模式**：在近日点和远日点对称绘制两个平近点角跨度相同的扇形扫过面积（`SCENE_COLORS.effects.sectorFill`），当行星运行进入扇形区域时动态点亮该面积，展示相同时间内扫过面积相同、近日点速度快、远日点速度慢的特征。
   - **第三定律模式（T² - a³ 对角直线图表画中画）**：同屏渲染行星 A 和行星 B，并在 Canvas 右下角新增图表画中画，以动态自适应量程使 $T^2 - a^3$ 曲线折叠成完美的斜率对角直线，行星 A 和行星 B 的状态点在直线上同步平滑移动，极大丰富教学层级。
   - **矢量箭头联动**：精确画出沿轨道切线的速度矢量 $v$（经典蓝）和指向太阳焦点的万有引力矢量 $F$（引力绿，长度正比于 $1/r^2$）。
   - **大标题展示**：通过 `showFormulas` 联动控制 Canvas 左上角大标题与局部状态参数在白底上的精密科学呈现。

6. **编写开普勒运动单元测试**（`tests/physics/celestial.test.ts`）
   - 编写了 4 个单元测试，包含牛顿迭代收敛性校验、焦半径之和恒定（第一定律）校验，以及近日/远日点角动量守恒极值（第二定律）校验，使用 Vitest 测试全部通过。
