...
| 日期 | 模块 | 类型 | 变更 |
|------|------|------|------|
| 09-24 | 工程/门禁 | compliance | **门禁三层补齐**：① **硬编码颜色自动门禁**——`eslint.config.mjs` 新增 `no-restricted-syntax`（`Literal[value=/^#[0-9A-Fa-f]{3,8}$/]`，作用域 `src/features/**` + `src/components/**`；`src/theme/**` 是颜色 token 的 SSOT 故排除），存量 **504 处 / 48 文件**经 `eslint --suppress-all` 冻结为 `eslint-suppressions.json` 基线（B1 基线抑制），新增违规立即拦（实测：新文件命中 1 处即报错；已冻结文件超出计数时该文件违规全量报出）；② **6 个守门脚本**（4.5s，全绿）接入 `lefthook` pre-commit 与 CI `npm run check:architecture`，此前二者均未覆盖、仅在手动执行时生效；③ 全量门禁实测 **35.05s**：guards 4.36 / lint 10.96 / test 7.78（926 通过）/ typecheck 11.94。注：存量清理一处后需 `eslint --prune-suppressions` 同步收紧基线 |
| 09-24 | 工程/门禁 | fix | **lefthook pre-commit glob 缺陷修复**：`glob: "*.{ts,tsx}"` 的 `*` 不跨目录分隔符，`src/` 下文件全部匹配不上，三次 commit 均输出 `lint/typecheck/test (skip) no matching staged files`，三道门禁形同虚设。改用 `{**/,}*.{...}` 同时覆盖子目录与仓库根目录（`**/*` 方案匹配不到根目录 `vite.config.ts`，故弃用）。实测 `src/hooks/useSceneScale.ts` 与根目录 `vite.config.ts` 均能触发，全量门禁 926 单测全绿。注：命令体为全仓 `eslint .` / `tsc --noEmit`，本次仅修复「是否触发」，未改变检查范围 |
