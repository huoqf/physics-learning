...
export const mechanicsCircularGravitationAnimations = defineAnimations({
  ...
  'anim-centripetal': {
    ...
    controlsMode: 'loop',
    defaultParams: { r: 3, v: 3, m: 1, showWaveform: 1, showAcceleration: 1 } as const,
    paramMeta: [
  ...
  'anim-vertical-circular': {
    title: '竖直圆模型',
    knowledgeId: 'mechanics-5-6',
    Component: lazy(() => import('@/features/mechanics/circular/VerticalCircularAnimation')),
    controlsMode: 'timed',
    defaultParams: { r: 3, v0: 5, m: 1, trackType: 0, showAcceleration: 1 } as const,
    paramMeta: [
      { key: 'r', label: '半径 r', min: 3, max: 5, step: 0.1, unit: 'm' },
      { key: 'v0', label: '最低点初速度 v₀', min: 1, max: 15, step: 0.5, unit: 'm/s' },
      { key: 'm', label: '质量 m', min: 0.5, max: 5, step: 0.1, unit: 'kg' },
    ],
    controlMeta: [
      ...
      { type: 'toggle', key: 'showAcceleration', label: '显示加速度矢量', group: '显示辅助' },
      { type: 'tip', group: '教学提示',
        content: '绳模型：单面约束，速度不足时会脱离轨道做抛体运动。杆模型：双面约束，可提供支持力。' },
    ],
  },
  ...
  'anim-orbit-transfer': {
    ...
    controlMeta: [
      ...
      },
      {
        type: 'storeToggle',
        label: '显示速度矢量',
        storeKey: 'toggleVectors',
        stateKey: 'showVectors',
        group: '显示辅助',
      },
      {
        type: 'tip',
      ...
      },
      {
        ...
        showIfValue: 1,
        variant: 'warning',
        content: '椭圆爬升段速度连续减小——不是"越飞越快"。等到远地点提示出现后再点「再次点火」。',
      },
    ],
  },
})
