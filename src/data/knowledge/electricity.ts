import type { KnowledgeNode } from '../types'

export const electricityKnowledge: KnowledgeNode[] = [
  {
    id: 'electricity-1-1',
    title: '库仑定律',
    chapter: '电磁学 第1章 静电场',
    module: 'electricity',
    importance: 'gaokao',
    animationIds: ['anim-coulomb-law'],
    problemIds: [],
    prerequisites: [],
  },
  {
    id: 'electricity-1-2',
    title: '电场强度 E=F/q',
    chapter: '电磁学 第1章 静电场',
    module: 'electricity',
    importance: 'gaokao',
    animationIds: ['anim-electric-field'],
    problemIds: [],
    prerequisites: ['electricity-1-1'],
  },
  {
    id: 'electricity-1-3',
    title: '带电粒子在匀强电场中运动',
    chapter: '电磁学 第1章 静电场',
    module: 'electricity',
    importance: 'gaokao',
    animationIds: ['anim-charge-in-efield'],
    problemIds: [],
    prerequisites: ['electricity-1-2'],
  },
  {
    id: 'electricity-1-4',
    title: '电容器（C=Q/U）',
    chapter: '电磁学 第1章 静电场',
    module: 'electricity',
    importance: 'core',
    animationIds: ['anim-capacitor'],
    problemIds: [],
    prerequisites: ['electricity-1-2'],
  },
  {
    id: 'electricity-1-5',
    title: '电场线与等势面关系',
    chapter: '电磁学 第1章 静电场',
    module: 'electricity',
    importance: 'gaokao',
    animationIds: ['anim-field-lines'],
    problemIds: [],
    prerequisites: ['electricity-1-2'],
  },
  {
    id: 'electricity-1-6',
    title: '电势与电势差',
    chapter: '电磁学 第1章 静电场',
    module: 'electricity',
    importance: 'gaokao',
    animationIds: ['anim-electric-potential'],
    problemIds: [],
    prerequisites: ['electricity-1-2'],
  },
  {
    id: 'electricity-1-7',
    title: '等量电荷场线模型',
    chapter: '电磁学 第1章 静电场',
    module: 'electricity',
    importance: 'hard',
    animationIds: [],
    problemIds: [],
    prerequisites: ['electricity-1-5'],
  },
  {
    id: 'electricity-1-8',
    title: '静电的防止与利用（静电平衡与静电屏蔽）',
    chapter: '电磁学 第1章 静电场',
    module: 'electricity',
    importance: 'gaokao',
    animationIds: ['anim-electrostatic-shielding'],
    problemIds: [],
    prerequisites: ['electricity-1-2'],
  },
  {
    id: 'electricity-2-1',
    title: '部分电路欧姆定律与电表改装',
    chapter: '电磁学 第2章 恒定电流',
    module: 'electricity',
    importance: 'gaokao',
    animationIds: ['anim-ohm-law'],
    problemIds: [],
    prerequisites: ['electricity-1-2'],
  },
  {
    id: 'electricity-2-2',
    title: '串并联电路及电路动态分析',
    chapter: '电磁学 第2章 恒定电流',
    module: 'electricity',
    importance: 'gaokao',
    animationIds: ['anim-circuit-analysis'],
    problemIds: [],
    prerequisites: ['electricity-2-1'],
  },
  {
    id: 'electricity-2-3',
    title: '闭合电路欧姆定律',
    chapter: '电磁学 第2章 恒定电流',
    module: 'electricity',
    importance: 'gaokao',
    animationIds: ['anim-closed-circuit'],
    problemIds: [],
    prerequisites: ['electricity-2-1'],
  },
  {
    id: 'electricity-2-4',
    title: '多用电表与欧姆表原理',
    chapter: '电磁学 第2章 恒定电流',
    module: 'electricity',
    importance: 'gaokao',
    animationIds: ['anim-multimeter-ohm'],
    problemIds: [],
    prerequisites: ['electricity-2-2', 'electricity-2-3'],
  },
  {
    id: 'electricity-2-5',
    title: '高考实验：测定电源电动势与内阻',
    chapter: '电磁学 第2章 恒定电流',
    module: 'electricity',
    importance: 'gaokao',
    animationIds: ['anim-experiment-er'],
    problemIds: [],
    prerequisites: ['electricity-2-3'],
  },
  {
    id: 'electricity-2-6',
    title: '非纯电阻电路与电动机',
    chapter: '电磁学 第2章 恒定电流',
    module: 'electricity',
    importance: 'gaokao',
    animationIds: ['anim-motor-circuit'],
    problemIds: [],
    prerequisites: ['electricity-2-3'],
  },
  {
    id: 'electricity-3-1',
    title: '安培力 F=BIL',
    chapter: '电磁学 第3章 磁场',
    module: 'electricity',
    importance: 'gaokao',
    animationIds: ['anim-ampere-force'],
    problemIds: [],
    prerequisites: ['electricity-2-3'],
  },
  {
    id: 'electricity-3-2',
    title: '洛伦兹力 F=qvB',
    chapter: '电磁学 第3章 磁场',
    module: 'electricity',
    importance: 'gaokao',
    animationIds: ['anim-lorentz-force'],
    problemIds: [],
    prerequisites: ['electricity-3-1'],
  },
  {
    id: 'electricity-3-3',
    title: '带电粒子在磁场中运动',
    chapter: '电磁学 第3章 磁场',
    module: 'electricity',
    importance: 'gaokao',
    animationIds: ['anim-charge-in-bfield'],
    problemIds: [],
    prerequisites: ['electricity-3-2'],
  },
  {
    id: 'electricity-3-3-geom',
    title: '磁场圆周几何模型',
    chapter: '电磁学 第3章 磁场',
    module: 'electricity',
    importance: 'gaokao',
    animationIds: ['anim-magnetic-circular-geom'],
    problemIds: [],
    prerequisites: ['electricity-3-3'],
  },
  {
    id: 'electricity-3-4',
    title: '复合场组合场模型',
    chapter: '电磁学 第3章 磁场',
    module: 'electricity',
    importance: 'hard',
    animationIds: ['anim-combined-fields'],
    problemIds: [],
    prerequisites: ['electricity-3-2', 'electricity-1-3'],
  },
  {
    id: 'electricity-4-1',
    title: '电磁感应现象与产生条件',
    chapter: '电磁学 第4章 电磁感应',
    module: 'electricity',
    importance: 'gaokao',
    animationIds: ['anim-electromagnetic-induction'],
    problemIds: [],
    prerequisites: ['electricity-3-1'],
  },
  {
    id: 'electricity-4-2',
    title: '楞次定律',
    chapter: '电磁学 第4章 电磁感应',
    module: 'electricity',
    importance: 'gaokao',
    animationIds: ['anim-lenzs-law'],
    problemIds: [],
    prerequisites: ['electricity-4-1'],
  },
  {
    id: 'electricity-4-3',
    title: '法拉第电磁感应定律',
    chapter: '电磁学 第4章 电磁感应',
    module: 'electricity',
    importance: 'gaokao',
    animationIds: ['anim-faraday-law'],
    problemIds: [],
    prerequisites: ['electricity-4-2'],
  },
  {
    id: 'electricity-4-4',
    title: '导线切割磁力线',
    chapter: '电磁学 第4章 电磁感应',
    module: 'electricity',
    importance: 'gaokao',
    animationIds: ['anim-cutting-emf'],
    problemIds: [],
    prerequisites: ['electricity-4-3'],
  },
  {
    id: 'electricity-4-5',
    title: '电磁感应单杆模型',
    chapter: '电磁学 第4章 电磁感应',
    module: 'electricity',
    importance: 'gaokao',
    animationIds: ['anim-induction-single-rod'],
    problemIds: [],
    prerequisites: ['electricity-4-4'],
  },
  {
    id: 'electricity-4-6',
    title: '电磁感应双杆模型',
    chapter: '电磁学 第4章 电磁感应',
    module: 'electricity',
    importance: 'hard',
    animationIds: ['anim-induction-dual-rods'],
    problemIds: [],
    prerequisites: ['electricity-4-5'],
  },
  {
    id: 'electricity-4-7',
    title: '电磁感应线框模型',
    chapter: '电磁学 第4章 电磁感应',
    module: 'electricity',
    importance: 'hard',
    animationIds: ['anim-induction-loop-field'],
    problemIds: [],
    prerequisites: ['electricity-4-4'],
  },
  {
    id: 'electricity-4-8',
    title: '自感与互感',
    chapter: '电磁学 第4章 电磁感应',
    module: 'electricity',
    importance: 'gaokao',
    animationIds: ['anim-self-induction-eddy'],
    problemIds: [],
    prerequisites: ['electricity-4-2', 'electricity-4-3'],
  },
  {
    id: 'electricity-4-9',
    title: '涡流、电磁阻尼与电磁驱动',
    chapter: '电磁学 第4章 电磁感应',
    module: 'electricity',
    importance: 'gaokao',
    animationIds: ['anim-self-induction-eddy'],
    problemIds: ['prob-2024-hubei-01'],
    prerequisites: ['electricity-4-8'],
  },
  {
    id: 'electricity-5-1',
    title: '交变电流产生与图像',
    chapter: '电磁学 第5章 交变电流',
    module: 'electricity',
    importance: 'core',
    animationIds: ['anim-ac-generation'],
    problemIds: [],
    prerequisites: ['electricity-4-3'],
  },
  {
    id: 'electricity-5-2',
    title: '有效值与峰值关系',
    chapter: '电磁学 第5章 交变电流',
    module: 'electricity',
    importance: 'gaokao',
    animationIds: ['anim-ac-values'],
    problemIds: [],
    prerequisites: ['electricity-5-1'],
  },
  {
    id: 'electricity-5-3',
    title: '变压器原理',
    chapter: '电磁学 第5章 交变电流',
    module: 'electricity',
    importance: 'gaokao',
    animationIds: ['anim-transformer'],
    problemIds: [],
    prerequisites: ['electricity-5-1'],
  },
  {
    id: 'electricity-5-4',
    title: '远距离输电',
    chapter: '电磁学 第5章 交变电流',
    module: 'electricity',
    importance: 'core',
    animationIds: ['anim-power-transmission'],
    problemIds: [],
    prerequisites: ['electricity-5-3'],
  },
  {
    id: 'electricity-5-5',
    title: '电感与电容对交变电流的作用',
    chapter: '电磁学 第5章 交变电流',
    module: 'electricity',
    importance: 'gaokao',
    animationIds: ['anim-ac-lc-impedance'],
    problemIds: [],
    prerequisites: ['electricity-5-1'],
  },
  // ── 第6章 电磁振荡与电磁波（人教版选择性必修第二册 第4章）──────────────
  {
    id: 'electricity-6-1',
    title: '电磁振荡（LC 振荡电路）',
    chapter: '电磁学 第6章 电磁振荡与电磁波',
    module: 'electricity',
    importance: 'gaokao',
    animationIds: ['anim-lc-oscillation'],
    problemIds: ['prob-2022-quanguo-34-1'],
    prerequisites: ['electricity-1-4', 'electricity-4-2'],
  },
  {
    id: 'electricity-6-2',
    title: '麦克斯韦电磁场理论与电磁波',
    chapter: '电磁学 第6章 电磁振荡与电磁波',
    module: 'electricity',
    importance: 'core',
    animationIds: ['anim-em-wave'],
    problemIds: [],
    prerequisites: ['electricity-6-1'],
  },
  {
    id: 'electricity-6-3',
    title: '电磁波谱与无线电波的发射与接收',
    chapter: '电磁学 第6章 电磁振荡与电磁波',
    module: 'electricity',
    importance: 'gaokao',
    animationIds: ['anim-em-spectrum'],
    problemIds: [],
    prerequisites: ['electricity-6-2'],
  },
  // ── 第7章 传感器（人教版选择性必修第二册 第3章）──────────────
  {
    id: 'electricity-7-1',
    title: '常见传感器及其工作原理',
    chapter: '电磁学 第7章 传感器',
    module: 'electricity',
    importance: 'gaokao',
    animationIds: ['anim-sensors'],
    problemIds: [],
    prerequisites: ['electricity-2-1', 'electricity-3-2'],
  },
  {
    id: 'electricity-7-2',
    title: '传感器的应用与自动控制',
    chapter: '电磁学 第7章 传感器',
    module: 'electricity',
    importance: 'gaokao',
    animationIds: ['anim-sensors'],
    problemIds: [],
    prerequisites: ['electricity-7-1'],
  },
]
