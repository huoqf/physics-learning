...
export function KineticEnergyCharts({
  ...
  const ekVisible = useMemo(() => visiblePoints.map((p) => ({ x: p.x, y: p.Ek })), [visiblePoints]);
  ...
  const epVisible = useMemo(() => visiblePoints.map((p) => ({ x: p.x, y: p.Ep })), [visiblePoints]);
  ...
  const ekChart = (
    <RelationChart
      ...
      markers={[
        { axis: 'vertical', x: criticalX, label: criticalLabel, color: CHART_COLORS.criticalPt },
      ]}
    />
  );
