      ...
      {isAdvanced && (
        <div className="mt-3 space-y-3">
          ...
          <div className="flex items-center justify-between text-xs py-1">
            <span className="text-neutral-600">简谐运动投影对比</span>
            <button
              type="button"
              disabled={disabled}
              onClick={() => updateParam('showProjection', (params.showProjection ?? 1) === 1 ? 0 : 1)}
              className={[
                'relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none',
                (params.showProjection ?? 1) === 1 ? 'bg-primary-600' : 'bg-neutral-200',
                disabled && 'opacity-40 pointer-events-none'
              ].join(' ')}
            >
              <span
                className={[
                  ...
                  (params.showProjection ?? 1) === 1 ? 'translate-x-4' : 'translate-x-0'
                ].join(' ')}
              />
            </button>
          </div>
          ...
          <div className="flex items-center justify-between text-xs py-1">
            <span className="text-neutral-600">显示联动波形图</span>
            <button
              type="button"
              disabled={disabled}
