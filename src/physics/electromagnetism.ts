...
export function calculateLorentzTrajectory(
  ...
  const r = Math.abs((m * v0) / (q * B))
  
  // 圆周运动轨迹：x = R*sin(ωt), y = R*(1-cos(ωt))
  const x = r * Math.sin(omega * t)
  const y = r * (1 - Math.cos(omega * t))
  
  // 速度：vx = dx/dt = Rωcos(ωt), vy = dy/dt = Rωsin(ωt)
  const vx = r * omega * Math.cos(omega * t)
...
function solveTOut(v0: number, vd: number, omega: number, L: number): number {
  let tMin = 0
  let tMax = Math.max(5.0, (2 * L) / Math.max(0.1, Math.min(v0, vd)))
  for (let i = 0; i < 25; i++) {
...
export function calculateVelocitySelectorTrajectory(
  ...
  const omega = m > 0 ? (q * B_val) / m : 0
  const vd = B_val > 0.001 ? E_val / B_val : 0
  ...
  if (B_val < 1e-6) {
    // 纯电场类平抛运动
    if (E_val > 1e-6 && Math.abs(q) > 1e-9 && m > 0) {
      const a_y_mag = (Math.abs(q) * E_val) / m
      tHit = Math.sqrt(halfD / (0.5 * a_y_mag))
      hitType = q < 0 ? 'top' : 'bottom' // E向下。正电荷受力向下(bottom)，负电荷受力向上(top)
      hitsPlate = true
    }
  } else {
    // 匀强磁场或复合正交电磁场
    if (Math.abs(v0 - vd) > 1e-4 && Math.abs(omega) > 1e-6) {
      const amp = (v0 - vd) / omega
      const tHitsList: { t: number; type: 'top' | 'bottom' }[] = []
      
      // 上极板 (y = +halfD)
      const cosValTop = 1 - halfD / amp
      if (cosValTop >= -1 && cosValTop <= 1) {
        const tVal = Math.acos(cosValTop) / Math.abs(omega)
        if (tVal > 0) {
          tHitsList.push({ t: tVal, type: 'top' })
        }
      }
      
      // 下极板 (y = -halfD)
      const cosValBottom = 1 - (-halfD) / amp
      if (cosValBottom >= -1 && cosValBottom <= 1) {
        const tVal = Math.acos(cosValBottom) / Math.abs(omega)
        if (tVal > 0) {
          tHitsList.push({ t: tVal, type: 'bottom' })
        }
      }

      if (tHitsList.length > 0) {
        tHitsList.sort((a, b) => a.t - b.t)
        tHit = tHitsList[0].t
        hitType = tHitsList[0].type
        hitsPlate = true
      }
    }
  }
  ...
  if (B_val >= 1e-6 && Math.abs(omega) > 1e-6) {
  ...
  if (tEffective <= 0) {
    ...
    if (B_val < 1e-6) {
      px_out = v0 * tOut
      py_out = -0.5 * ((q * E_val) / m) * tOut * tOut
      vx_out = v0
      vy_out = -((q * E_val) / m) * tOut
    } else {
      px_out = vd * tOut + ((v0 - vd) / omega) * Math.sin(omega * tOut)
      py_out = ((v0 - vd) / omega) * (1 - Math.cos(omega * tOut))
    ...
    if (B_val < 1e-6) {
      px = v0 * tEffective
      py = -0.5 * ((q * E_val) / m) * tEffective * tEffective
      vx = v0
      vy = -((q * E_val) / m) * tEffective
    } else {
      px = vd * tEffective + ((v0 - vd) / omega) * Math.sin(omega * tEffective)
      py = ((v0 - vd) / omega) * (1 - Math.cos(omega * tEffective))
  ...
  const fx = isInside && B_val > 0 ? -q * B_val * vy : 0
  const fy = isInside && B_val > 0 ? q * B_val * vx : 0
  
  const fEx = 0
  const fEy = isInside && E_val > 0 ? -q * E_val : 0
