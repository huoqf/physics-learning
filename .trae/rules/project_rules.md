...
| preset | 尺寸 | 布局特征 |
|---|---|---|
| `tall` | 700×450 | 标准宽幅，适合上下分区或含较高标注区域 |
| `standard` | 700×420 | 标准宽幅，适合单场景 + 右侧信息面板 |
| `mediumTall` | 650×450 | 略窄高幅，适合居中对称构图 |
| `wide` | 700×400 | 标准宽幅，适合横向运动或左右分区 |
| `mediumWide` | 650×400 | 略窄标准，适合紧凑场景或子组件 |
| `square` | 600×600 | 正方形，适合旋转/圆形轨迹 |
| `extraWide` | 800×440 | 超宽幅，适合复杂场景或多区域并排 |

> 新增动画组件应根据教学内容的布局需求选择合适的 preset，而非按物理主题固定映射。仅当布局有特殊需求（占位符/紧凑子场景/唯一比例）时允许硬编码 `{ width, height }`。
