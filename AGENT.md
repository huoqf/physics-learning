...
### 动画映射表（优先于 codebase-memory）

查询动画相关代码时，**优先使用 `MEMORY.md` 中的动画映射表**，而不是 codebase-memory 图谱。

原因：
- codebase-memory 基于 LSP 静态分析，无法正确追踪 `lazy(() => import(...))` 的调用链
- 动画映射表由脚本 `node scripts/gen-annotations.mjs` 自动生成，准确反映 registry → entry → scene → physics 的实际调用关系
